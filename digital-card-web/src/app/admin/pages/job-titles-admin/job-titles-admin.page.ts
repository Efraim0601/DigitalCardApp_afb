    ...
    <div class="space-y-5">
      ...
      @if (showForm()) {
        <div class="rounded-xl border border-slate-200 bg-white p-5">
          <div class="text-[17px] font-semibold text-slate-900">Ajouter un titre / poste</div>
          @if (error()) {
          ...
          <table class="min-w-[600px] w-full border-collapse">
            ...
            <tbody>
              @if (isLoading()) {
                ...
                <tr>
                  <td colspan="3" class="px-4 py-6 text-center text-sm text-slate-400">Aucun titre / poste enregistré.</td>
                </tr>
              } @else {
...
export class JobTitlesAdminPageComponent {
  ...
  load() {
    ...
    this.admin
      .listJobTitles({ q: this.q() || undefined, limit: this.pageSize, offset: (this.page() - 1) * this.pageSize })
      .pipe(
        tap((res) => {
        ...
        catchError(() => {
          this.error.set('Impossible de charger les titres.');
          return of(null);
        }),
  ...
  save() {
    ...
    this.isLoading.set(true);
    this.admin
      .createJobTitle({ labelFr: this.form.controls.labelFr.value.trim(), labelEn: this.form.controls.labelEn.value.trim() })
      .pipe(
